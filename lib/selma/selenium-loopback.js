define([], function() {
	...
	function wrapPromise(promise, object)
	{
		for (var n in object) {
			var f = object[n];
			if (typeof f === "function") {
				f = f.bind(object);
				promise[n] = function() {
					return wrapPromise(promise.then(f.call([].slice.call(arguments))), object);
				};
			} else {
				promise[n] = f;
			}
	...
	function transformElement(webElement) {
		...
		return {
			...
			click: function() { return wrapPromise(post(prefix + 'click'), this); },
			submit: function() { return wrapPromise(post(prefix + 'submit'), this); },
			text: function() { return get(prefix + 'text'); },
			keys: function(keys) { return wrapPromise(post(prefix + 'value', keys), this); },
			...
			equals: function(otherElement) { return gete(prefix + 'equals/' + otherElement.id); },
	...
	function get2dContext(image, w, h) {
		...
		context.pixelAt = function(x, y) { return [].slice.call(context.getImageData(x * devicePixelRatio, y * devicePixelRatio, 1, 1).data); }
	...
	function Selenium(promise) {
		function wrap(f) {
			return Selenium(promise.then(f));
		}
		...
		var s = {
			...
			canvas: function(w, h) { return screenshot().then(function(image) {
				return get2dContext(image, w, h);
			}); },
			...
			keys: function(keys) { return postPromise('keys', keys); },
			...
			then: function(success, fail) { return Selenium(promise.then(success, fail)); },
			flush: flush
		};
